...
def test_list_recipes_by_ingredient():
    ...
    response = client.post("/recipe/search/", json=data)
    assert response.status_code == 200, "Expected 200, got "
...
def test_list_ingredients():
    ...
    query_string = "to"
    response = client.get(
        f"/recipe/ingredients/{query_string}",
        params={
...
def test_recommend_recipes():
    ...
    query_data = {"query": "easy pasta recipe",
...
def test_find_recipe_non_existent_id():
    ...
    assert response.status_code == 404, "Expected 404, got "
    f"{response.status_code}: {response.text}"
    assert "detail" in response.json()
...
def test_create_user_recipe(test_user_id):
    ...
    recipe_data = {
        ...
        # Added to match required field, as strings
        "ingredientQuantities": [1.0]
    }
    response = client.post(
        f"/recipe/user/{test_user_id}",
        params={
...
def test_remove_from_meal_plan(test_user_id):
    ...
    client.put(
        ...
        params={
            "user_id": test_user_id},
        json=entry)  # Add entry first
    response = client.request("POST",
                              f"/meal-plan/delete/{test_user_id}",
                              params={"user_id": test_user_id},
                              json=2)  # Send day as plain integer
    assert response.status_code == 200, "Expected 200, got "
    f"{response.status_code}: {response.text}"
...
def test_get_empty_meal_plan(test_user_id):
    ...
    meal_plan = response.json()
    assert isinstance(
        meal_plan, list), "Expected meal_plan to be a list, "
    f"but got: {meal_plan}"
...
def test_favorite_recipe(test_user_id):
    ...
    response = client.put(
        f"/recipe/favorite/1/{test_user_id}",
        params={
            "recipe_id": 1,
            "user_id": test_user_id})
    assert response.status_code == 200, "Expected 200, got "
...
def test_unfavorite_recipe(test_user_id):
    ...
    client.put(
        ...
        params={
            ...
            "user_id": test_user_id})
    response = client.put(
        f"/recipe/unfavorite/1/{test_user_id}",
        params={
            "recipe_id": 1,
            "user_id": test_user_id})
    print(response.json())
